...
window.MobileSelect = (function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			...
			_this.showValue = document.querySelector(config.showValue);
			...
			var tempValue ='';
		    	...
		    	_this.showValue.innerHTML = tempValue;
			...
			for(var i=0; i<wheelsData.length; i++){ //列
				...
				if(_this.jsonType){
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
						tempHTML += '<li data-id="'+wheelsData[i].data[j].c+'">'+wheelsData[i].data[j].n+'</li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){ //行
		...
		checkCascade: function(){
			...
			if(_this.jsonType){ 
				...
				for(var i=0; i<node.length; i++){
					if('s' in node[i] && node[i].s.length > 0){
						_this.cascade = true;
						break;
					}
				}
		...
		checkArrDeep: function (parent) { //检测子节点深度  修改 displayJson
			var _this = this;
			if ('s' in parent && parent.s.length > 0) {
				_this.displayJson.push(_this.generateArrData(parent.s)) ; //生成子节点数组
				_this.checkArrDeep(parent.s[0]);//检测下一个子节点
			}
		},
		...
		checkRange: function(index, posIndexArr){
			...
			for (var i = 0; i <= index; i++){
				if (i == 0)
					...
					resultNode = resultNode.s[posIndexArr[i]];
				}
			}
				...
				(function (i) {
					...
					if(_this.wheel[i]){
						...
						for(var j=0; j<_this.displayJson[i].length; j++){ //行
							tempHTML += '<li data-id="'+_this.displayJson[i][j].c+'">'+_this.displayJson[i][j].n+'</li>';
						}
